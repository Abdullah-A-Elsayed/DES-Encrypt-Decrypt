...
my $enc = &multi_decrypt ($long_key_hexa , $long_plain_hexa , 16, $repeat);
print $enc."\n";
...
sub multi_decrypt{
    ...
    for (my $i=0; $i<$repeat ; $i++){
        $long_plain_hexa = &decrypt ($long_key_hexa , $long_plain_hexa , 16);
    }
...
sub decrypt{
    ...
    for (my $i=0; $i<$rounds ; $i++){
        my $fn = &des_fn($RH, $round_keys[$rounds - $i -1]);
        my $temp = $RH;
        $RH = &xor_hexa($LH , $fn);
        $LH = $temp;
    }
